...
function getConfig() {
  const c = vscode.workspace.getConfiguration('merlin-remote');
...
function resolvePythonRuntime(root, output) {
  ...
  const hint = process.platform === 'win32'
    ? 'Set merlin-remote.pythonPath to a valid python.exe path.'
    : 'Set merlin-remote.pythonPath to a valid Python executable.';
...
class RemoteTrainProvider {
  constructor(root) {
    this.root = root;
    this.view = null;
    this.output = vscode.window.createOutputChannel('MERLIN Remote');
    this.chat = [
      {
        role: 'assistant',
  ...
  _getInitialHtml(webview) {
    ...
    return `<!DOCTYPE html><html><head>
      ...
      <style>${CSS}
        .scrollable { overflow-y:auto; padding:4px 0; }
      </style>
    </head><body>
      <div class="header">
      ...
      <div class="scrollable" id="mainContent"></div>
      <script nonce="${nonce}">
        const vscode = acquireVsCodeApi();
        function send(type, payload) { vscode.postMessage(Object.assign({ type: type }, payload || {})); }
        function selectBrain(brain) { send('selectBrain', { brain: brain }); }
        function sendBrain(action) { send(action, { brain: document.querySelector('[data-selected-brain]')?.dataset.selectedBrain || 'narrator' }); }
        ...
        function chat() {
          ...
          if (!prompt) return;
          send('chat', { prompt: prompt });
          el.value = '';
        }
        function quickHello() { send('chat', { prompt: 'Bonjour Merlin' }); }
        window.addEventListener('message', function(event) {
          var msg = event.data;
          if (msg.type === 'htmlUpdate' && msg.sections) {
            if (msg.sections.main) {
              var el = document.getElementById('mainContent');
              if (el) el.innerHTML = msg.sections.main;
            }
            if (msg.sections.headerStatus) {
              var hs = document.getElementById('headerStatus');
              if (hs) hs.innerHTML = msg.sections.headerStatus;
            }
          }
        });
      </script>
  ...
  resolveWebviewView(view) {
    ...
    // Kaggle state polling every 30s
  ...
  async configure() {
    ...
    const ws = vscode.workspace.getConfiguration('merlin-remote');
    ...
    vscode.window.showInformationMessage('MERLIN Remote config updated');
    this.update();
  }
  ...
  async openPanel() {
    await vscode.commands.executeCommand('workbench.view.extension.merlin-remote-sidebar');
    try {
      await vscode.commands.executeCommand('merlin-remote.panel.focus');
    } catch {
  ...
  update() {
    ...
    const statusColor = rawStatus === 'complete' ? CRT.green
      : rawStatus === 'running' ? CRT.cyan
      : rawStatus === 'error' ? CRT.red
      : CRT.gray;
    ...
    webview.postMessage({
      type: 'htmlUpdate',
      sections: {
        headerStatus: `<span style="color:${statusColor}">${escapeHtml(rawStatus.toUpperCase())}</span>`,
        main: this._moreHtml(state, cfg, trainDisabled, chatDisabled, endpointLabel, brainSummary, selBrainState, selJob)
      }
    });
  }
  ...
  _moreHtml(state, cfg, trainDisabled, chatDisabled, endpointLabel, brainSummary, selBrainState, selJob) {
    return `
      <div class="block" data-selected-brain="${escapeHtml(this.selectedBrain)}">
        ...
        <div class="btnRow">
          ...
          <button ${trainDisabled} onclick="send('openKaggle')">Kaggle</button>
        </div>
      </div>
      <div class="block">
      ...
      <div class="block">
        ...
        <div class="btnRow" style="margin-bottom:6px">
          <button onclick="send('presetTogether')">Together</button>
          <button onclick="send('presetGroq')">Groq</button>
          <button onclick="send('presetRunpod')">RunPod</button>
        </div>
        <div class="chatWrap">
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('merlin-remote.panel', provider),
    vscode.commands.registerCommand('merlin-remote.open', async () => provider.openPanel()),
    vscode.commands.registerCommand('merlin-remote.refresh', () => provider.refresh()),
    vscode.commands.registerCommand('merlin-remote.configure', async () => provider.configure()),
    vscode.commands.registerCommand('merlin-remote.doctor', async () => provider.runRemote('doctor')),
    vscode.commands.registerCommand('merlin-remote.setup', async () => provider.runRemote('setup')),
    vscode.commands.registerCommand('merlin-remote.submit', async () => provider.runRemote('submit')),
    vscode.commands.registerCommand('merlin-remote.status', async () => provider.runRemote('status')),
    vscode.commands.registerCommand('merlin-remote.download', async () => provider.runRemote('download')),
    vscode.commands.registerCommand('merlin-remote.openKaggle', () => provider.openKaggle())
  );
}
