...
static func _get_default_profile() -> Dictionary:
	return {
		...
		"fins_vues": 0,
		"faction_rep": {
			"druides": float(MerlinConstants.FACTION_SCORE_START),
			"anciens": float(MerlinConstants.FACTION_SCORE_START),
			"korrigans": float(MerlinConstants.FACTION_SCORE_START),
			"niamh": float(MerlinConstants.FACTION_SCORE_START),
			"ankou": float(MerlinConstants.FACTION_SCORE_START),
		},
		"trust_merlin": 0,
