...
## Réinitialise toutes les réputations à FACTION_SCORE_START (Neutre).
func reset() -> void:
...
## Retourne un dict factions initialisé à FACTION_SCORE_START (20.0 = Neutre).
static func build_default_factions() -> Dictionary:
	var result: Dictionary = {}
	for faction in FACTIONS:
		result[faction] = float(MerlinConstants.FACTION_SCORE_START)
	return result
