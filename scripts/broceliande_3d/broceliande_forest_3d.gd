...
signal merlin_encounter_complete  # Emitted when Merlin found → encounters handled inline
...
func _on_hub() -> void:
	# After forest walk, always return to HubAntre (encounters handled inline via EncounterCardOverlay)
	var pt: Node = get_node_or_null("/root/PixelTransition")
	if pt and pt.has_method("transition_to"):
