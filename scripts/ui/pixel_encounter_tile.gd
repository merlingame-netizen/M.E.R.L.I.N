...
const PALETTE := {
	...
	"outline": Color(0.22, 0.18, 0.14),
	"fill_1": Color(0.58, 0.44, 0.26),
	"fill_2": Color(0.45, 0.62, 0.32),
	"accent": Color(0.72, 0.50, 0.10),
	"highlight": Color(0.94, 0.88, 0.72),
	"danger": Color(0.78, 0.25, 0.22),
	"magic": Color(0.45, 0.35, 0.72),
	"water": Color(0.30, 0.55, 0.72),
}
...
func _draw_path_choice() -> void:
	...
	var c := PALETTE.fill_1
	var o := PALETTE.outline
	var g := PALETTE.fill_2
	# Tree canopy top
	for x in range(8, 16):
		_set_px(x, 2, g)
		_set_px(x, 3, g)
...
func _draw_rest() -> void:
	...
	var o := PALETTE.outline
	var f := PALETTE.accent
	var d := PALETTE.danger
	var w := PALETTE.highlight
	# Logs
	for x in range(7, 17):
		_set_px(x, 17, o)
		_set_px(x, 18, o)
