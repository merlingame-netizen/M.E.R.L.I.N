...
func _show_ogham_selector() -> void:
	_cleanup_ogham_selector()
	_ogham_selector = OghamSelector.new()
	add_child(_ogham_selector)
	_ogham_selector.ogham_selected.connect(_on_ogham_confirmed)
	_ogham_selector.selector_dismissed.connect(_on_ogham_dismissed)
	var last_ogham: String = ""
	if store:
		last_ogham = str(store.state.get("run", {}).get("ogham_actif", ""))
	_ogham_selector.open(last_ogham)


func _on_ogham_confirmed(ogham_key: String) -> void:
	if store:
...
func _cleanup_ogham_selector() -> void:
	if is_instance_valid(_ogham_selector):
		if _ogham_selector.ogham_selected.is_connected(_on_ogham_confirmed):
			_ogham_selector.ogham_selected.disconnect(_on_ogham_confirmed)
		if _ogham_selector.selector_dismissed.is_connected(_on_ogham_dismissed):
			_ogham_selector.selector_dismissed.disconnect(_on_ogham_dismissed)
		_ogham_selector.queue_free()
	_ogham_selector = null
