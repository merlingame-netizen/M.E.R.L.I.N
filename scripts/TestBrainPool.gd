...
func _launch_minigame(direction: String) -> void:
	...
	var field: String = MiniGameRegistry.detect_field(narrative, gm_hint)
	...
	var modifiers := {}
	var ogham_bonus: int = MiniGameRegistry.get_ogham_bonus("", field)
	if ogham_bonus > 0:
	...
	# Create mini-game
	var game: MiniGameBase = MiniGameRegistry.create_minigame(field, base_diff, modifiers)
	if game == null:
