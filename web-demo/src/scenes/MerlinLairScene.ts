...
function createLairDensity(scene: THREE.Scene): void {
  ...
  // 3 shelves on left wall with books
  for (const [y, z] of [[1.5, -2.0], [-0.5, 0.5], [2.5, 3.5]] as [number, number][]) {
    ...
    for (let b = 0; b < 5; b++) {
      ...
      const book = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, 0.55),
        new THREE.MeshStandardMaterial({ color: bC[b % bC.length]!, roughness: 0.8, metalness: 0.0 }));
      book.position.set(-11.55, y - 1.2 + bh / 2, z - 0.8 + b * 0.35); scene.add(book);
    }
  }
  // Moonlight shaft from window in left wall
  ...
  // Parchment rolls on floor
  const pM = new THREE.MeshStandardMaterial({ color: 0xd4b878, roughness: 0.9, metalness: 0.0 });
