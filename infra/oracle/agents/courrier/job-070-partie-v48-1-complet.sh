#!/usr/bin/env bash
# job-070 — LA PARTIE TEMOIN DE v48.1 COMPLET (les quatre moities), jugee sur les TROIS CIBLES DURES de Maxime :
#
...
dire() { curl -fsS -m 20 -H "Title: p70 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
# job-069 est parti AVANT que v48.1d et le correctif d'environnement n'atteignent la VM : son
# verdict ne vaut rien, et son marqueur .fait l'empeche de rejouer. job-070 est la vraie mesure,
# et il exige les QUATRE moities, plus le correctif qui fait passer le drapeau a travers unshare.
while true; do
    A=0; B=0; C=0; D=0; E=0
    ...
    grep -q "MERLIN_BOT_COUVRANT" "${REPO:-$HOME/workspace/M.E.R.L.I.N}/infra/oracle/game/game-stack.sh" 2>/dev/null && E=1
    [ "$A$B$C$D$E" = "11111" ] && break
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v48.1 incomplet : sonde=$A place=$B annul=$C draft=$D env=$E (jeu=$(git -C "$GD" rev-parse --short HEAD 2>/dev/null))"; exit 1; }
    sleep 30
done
...
if [ ! -s "$B/journal.json" ]; then
    tail -40 "$COURRIER_RES/partie.log" > "$COURRIER_RES/pourquoi70.txt"
...
python3 - "$B/journal.json" <<'PY' > "$COURRIER_RES/verdict70.txt"
import json, sys, re
...
dire "verdict" "captures=$npng sha=$SHA $(head -c 700 "$COURRIER_RES/verdict70.txt")"

# le journal complet part aussi : c'est lui qui porte les chiffres fins.
curl -fsS -m 60 --retry 2 -T "$COURRIER_RES/journal.json" \
    -H "Filename: p69_journal.json" -H "Title: p70 journal" "$NT" >/dev/null 2>&1

echo "job-070 : partie v48.1 jouee (sha=$SHA), $npng captures + journal dans COURRIER_RES."
