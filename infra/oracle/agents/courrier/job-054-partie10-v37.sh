#!/usr/bin/env bash
# Partie témoin v37 — priorité scène bornée + scène 65 tok.
# Cibles : lookahead >= 2 servies, SECOURS=0, duree_moy <= 55 s, issue_s_moy <= 35 s.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p54 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t54.
    local total i p
    total=$(ls /tmp/t54.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t54.* | sort); do
        i=$((i+1))
...
while ! grep -q "v37" "$GD/scripts/llm/merlin_scenario.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v37 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "temoin v37 : priorite scene" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
tranches "course54" "$D"
cp -f "$D" "$HOME/.cache/merlin-agents/course54.copie.txt" 2>/dev/null || true
tranches "journal54" "$B/journal.json"
echo "p54 : verdict + course + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
