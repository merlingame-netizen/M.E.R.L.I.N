...
dire() { curl -fsS -m 20 -H "Title: q87 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
cp -f "$LOG" "$COURRIER_RES/q87_log.txt" 2>/dev/null || : > "$COURRIER_RES/q87_log.txt"
{
...
} > "$COURRIER_RES/q87_extrait.txt" 2>&1

if [ -z "$TROUVE" ]; then
    dire "ko" "pas de quete apres ${DUREE}s (vu=$VU) — extrait + log entier en pieces jointes"
    for f in "$COURRIER_RES/q87_extrait.txt:q87_extrait.txt" "$COURRIER_RES/q87_log.txt:q87_log.txt"; do
        curl -fsS -m 120 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q87 ${f##*:}" "$NT" >/dev/null 2>&1
        sleep 3
    done
    exit 1
fi
...
python3 "$GD/tools/scenarios/valider.py" "$TROUVE" > "$COURRIER_RES/verdict87.txt" 2>&1
VERDICT=$?
...
for f in "$TROUVE:q87_quete.json" "$COURRIER_RES/verdict87.txt:q87_verdict.txt" "$COURRIER_RES/q87_extrait.txt:q87_extrait.txt"; do
    curl -fsS -m 90 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q87 ${f##*:}" "$NT" >/dev/null 2>&1
    sleep 2
done
echo "job-087 : ${DUREE}s, $DIAG"
