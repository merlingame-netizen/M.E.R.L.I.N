#!/usr/bin/env bash
# job-083 — L'ADRESSE ET L'ARC : la meme quete que q82, ecrite avec ce qui a ete corrige.
#
...
dire() { curl -fsS -m 20 -H "Title: q83 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
while true; do
    ...
    [ "$A$B$C" = "111" ] && break
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "incomplet : arc=$A controle_adresse=$B canon_vouvoye=$C"; exit 1; }
    sleep 30
done
...
tail -40 "$LOG" 2>/dev/null > "$COURRIER_RES/q83_log.txt"

if [ -z "$TROUVE" ]; then
    dire "ko" "pas de quete apres ${DUREE}s (vu=$VU) — log en piece jointe, non filtre"
    curl -fsS -m 90 -T "$COURRIER_RES/q83_log.txt" -H "Filename: q83_log.txt" -H "Title: q83 log" "$NT" >/dev/null 2>&1
    exit 1
fi

cp "$TROUVE" "$COURRIER_RES/quete_generee.json"
python3 "$GD/tools/scenarios/valider.py" "$TROUVE" > "$COURRIER_RES/verdict83.txt" 2>&1
VERDICT=$?
...
for f in "$TROUVE:q83_quete.json" "$COURRIER_RES/verdict83.txt:q83_verdict.txt" "$COURRIER_RES/q83_log.txt:q83_log.txt"; do
    curl -fsS -m 90 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q83 ${f##*:}" "$NT" >/dev/null 2>&1
    sleep 2
done
echo "job-083 : ${DUREE}s, $DIAG"
