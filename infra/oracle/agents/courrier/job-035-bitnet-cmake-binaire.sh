#!/usr/bin/env bash
# Banc BitNet, prise 3 : cmake BINAIRE officiel aarch64 (user-mode), puis build + banc.
set -u
NT="https://ntfy.sh/merlin-courrier-vX9k2Qf7Lw3s"
W="$HOME/.cache/bitnet-banc"
dire() { curl -fsS -m 20 -H "Title: bit35 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; }

CM="$HOME/.local/cmake"
if [ ! -x "$CM/bin/cmake" ]; then
    mkdir -p "$CM"
    curl -fSL -o /tmp/cmake.tgz \
      "https://github.com/Kitware/CMake/releases/download/v3.29.6/cmake-3.29.6-linux-aarch64.tar.gz" \
      >> "$COURRIER_RES/build.log" 2>&1 || { dire "ko" "telechargement cmake KO"; exit 1; }
    tar -xzf /tmp/cmake.tgz -C "$CM" --strip-components=1 || { dire "ko" "extraction cmake KO"; exit 1; }
    rm -f /tmp/cmake.tgz
fi
export PATH="$CM/bin:$HOME/.local/bin:$PATH"
command -v cmake >/dev/null 2>&1 || { dire "ko" "cmake binaire introuvable"; exit 1; }
dire "etape" "$(date -u +%H:%M:%SZ) $(cmake --version | head -1)"
...
bench() {
    ...
    for p in $(ls /tmp/bn.* | sort); do
        i=$((i+1))
        curl -fsS -m 30 -H "Title: bit35-$nom part $i/$total" --data-binary @"$p" "$NT" >/dev/null 2>&1
        sleep 2
    done
    rm -f /tmp/bn.*
}
