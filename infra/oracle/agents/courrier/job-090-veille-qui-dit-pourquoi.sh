#!/usr/bin/env bash
# job-090 — UNE VEILLE QUI DIT POURQUOI ELLE S'ARRETE.
#
# q89 est morte comme q84 : sept beats ecrits, puis le jeu tue pendant le huitieme. J'avais
# pourtant corrige la regle de vie apres q84. Elle est peut-etre encore fausse — mais je ne peux
# pas le savoir, parce que LA BOUCLE NE DIT PAS CE QUI L'A ARRETEE. Deux courses perdues sur une
# condition que je continue de deviner : ca suffit.
#
# La boucle inscrit desormais sa RAISON de sortie et l'envoie avec le verdict : fichier trouve,
# budget epuise, processus disparu avec log fige, ou jamais parti. Plus une trace a chaque tour
# ou pgrep manque, avec l'age du log au meme instant.
#
# Elle ne s'arrete PLUS sur pgrep seul. q89 a mesure qu'un beat tient 130 s sans ecrire une ligne
# — evaluation de 1500 jetons a 30 tok/s, puis ecriture de 400 a 5 tok/s. Une fenetre de silence
# de deux minutes etait donc trop courte : elle passe a six, et seul le budget global (2400 s)
# ou le fichier arretent la course.
#
# LE PROMPT REDESCEND AUSSI. q89 l'a mesure a 1500 jetons, plus 400 de reponse : 1900 sur une
# fenetre de 2048. C'est au bord, et j'avais annonce 96 jetons pour l'exemple en ne comptant que
# sa scene et son issue. Les issues precedentes sont tronquees et la regle d'ecriture des figures
# ne part plus : environ 1250 jetons, l'exemple garde.
...
dire() { curl -fsS -m 20 -H "Title: q90 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
while true; do
    ...
    [ "$A$B" = "11" ] && break
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "incomplet : exemple=$A prompt_allege=$B"; exit 1; }
    sleep 30
done
...
cp -f "$LOG" "$COURRIER_RES/q90_log.txt" 2>/dev/null || : > "$COURRIER_RES/q90_log.txt"
{
...
} > "$COURRIER_RES/q90_extrait.txt" 2>&1

if [ -z "$TROUVE" ]; then
    dire "ko" "pas de quete apres ${DUREE}s (vu=$VU) — VEILLE : $RAISON"
    for f in "$COURRIER_RES/q90_extrait.txt:q90_extrait.txt" "$COURRIER_RES/q90_log.txt:q90_log.txt"; do
        curl -fsS -m 120 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q90 ${f##*:}" "$NT" >/dev/null 2>&1
        sleep 3
    done
    exit 1
fi
...
python3 "$GD/tools/scenarios/valider.py" "$TROUVE" > "$COURRIER_RES/verdict90.txt" 2>&1
VERDICT=$?
...
for f in "$TROUVE:q90_quete.json" "$COURRIER_RES/verdict90.txt:q90_verdict.txt" "$COURRIER_RES/q90_extrait.txt:q90_extrait.txt"; do
    curl -fsS -m 90 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q90 ${f##*:}" "$NT" >/dev/null 2>&1
    sleep 2
done
echo "job-090 : ${DUREE}s, $DIAG"
