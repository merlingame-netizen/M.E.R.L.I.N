#!/usr/bin/env bash
# Partie témoin v42 — le Voyageur sans passé inventé, le canon qui borne le monde.
# Cibles : passe=0 (aucune allusion à un passé), SECOURS=0, beat1 <= 50 s, duree_moy <= 45 s.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p61 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t61.
    local total i p
    total=$(ls /tmp/t61.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t61.* | sort); do
        i=$((i+1))
...
while ! grep -q "LORE_CANON" "$GD/scripts/llm/merlin_prompt_builder.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v42 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "temoin v42 : sans passe invente" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
[ -s "$B/journal.json" ] || { dire "ko" "journal absent rc=$RCP : $(tail -c 350 "$COURRIER_RES/partie.log" | tr '\n' ' ')"; exit 1; }

python3 - "$B/journal.json" "$COURRIER_RES/passe61.txt" <<'PY' > "$COURRIER_RES/verdict61.txt"
import json, re, sys
d = json.load(open(sys.argv[1]))
bs = d.get("beats") or []
resolus = [b for b in bs if "degre" in b]
# Motifs INTERDITS : toute allusion a un passe du Voyageur (v42).
MOTIFS = [
...
textes = []
for s in (d.get("sentiers") or []):
    textes.append(("pitch", str(s.get("pitch", ""))))
...
fin = d.get("fin") or {}
print("beats=%d passe=%d SECOURS=%d prov=%s vous=%d/%d beat1=%.0fs duree_moy=%.0fs issue_s_moy=%.0fs gw=%s fin=%s corr=%s titre=%s" % (
    len(bs), len(fautes), sec, ",".join("%s:%d" % kv for kv in sorted(prov.items())),
    vous, len(resolus),
...
PY
dire "verdict" "rc=$RCP $(cat "$COURRIER_RES/verdict61.txt")"
tranches "passe61" "$COURRIER_RES/passe61.txt"
tranches "journal61" "$B/journal.json"
echo "p61 : verdict + allusions + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
