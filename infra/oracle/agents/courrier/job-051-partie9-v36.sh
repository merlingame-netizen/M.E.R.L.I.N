#!/usr/bin/env bash
# Partie témoin v36 — l'issue directe : 2-3 phrases sèches, verbe joué, zéro image.
# Cibles : issues ≤ ~115 tok et ≤ ~40 s, SECOURS=0, lookahead ≥ 1 maintenu.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p51 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t51.
    local total i p
    total=$(ls /tmp/t51.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t51.* | sort); do
        i=$((i+1))
...
while ! grep -q "v36" "$GD/scripts/llm/merlin_prompt_builder.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v36 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "temoin v36 : issue directe, verbe joue" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
fin = d.get("fin") or {}
print("beats=%d SECOURS=%d prov=%s duree_moy=%.0fs issue_tok_moy=%.0f issue_s_moy=%.0fs fin=%s corr=%s" % (
...
tranches "course51" "$D"
tranches "journal51" "$B/journal.json"
echo "p51 : verdict + course + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
