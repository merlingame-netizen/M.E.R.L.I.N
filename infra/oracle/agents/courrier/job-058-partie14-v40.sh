#!/usr/bin/env bash
# Partie témoin v40 — écriture au Voyageur, colporteur narré, économie visible.
# Cibles : vous=6/6, SECOURS=0, gwenneg par beat présent, rythme tenu (beat1 <= 50 s,
# duree_moy <= 45 s malgré d'éventuels re-essais « Vous »).
set -u
...
dire() { curl -fsS -m 20 -H "Title: p58 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t58.
    local total i p
    total=$(ls /tmp/t58.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t58.* | sort); do
        i=$((i+1))
...
while ! grep -q "v40" "$GD/scripts/game/merlin_game.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v40 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "temoin v40 : Voyageur, colporteur, economie" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
vous = 0
for b in resolus:
    t = str(b.get("resolution", "")).strip()
    t = re.sub(r"^\[i\]\s*", "", t)
    t = re.sub(r"^[*\s]+", "", t)
    if t.startswith("Vous") or t.startswith("vous"):
        vous += 1
gw = "/".join(str(b.get("gwenneg_apres", "?")) for b in resolus)
fin = d.get("fin") or {}
print("beats=%d SECOURS=%d prov=%s vous=%d/%d beat1=%.0fs duree_moy=%.0fs issue_s_moy=%.0fs gw=%s fin=%s corr=%s titre=%s" % (
...
echo "== re-essais Vous ==" > "$COURRIER_RES/course58.txt"
grep -a "re-essai" "$L" >> "$COURRIER_RES/course58.txt" 2>/dev/null
tranches "course58" "$COURRIER_RES/course58.txt"
tranches "journal58" "$B/journal.json"
echo "p58 : verdict + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
