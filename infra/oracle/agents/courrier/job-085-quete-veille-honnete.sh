#!/usr/bin/env bash
# job-085 — UNE VEILLE QUI NE TUE PAS CE QU'ELLE SURVEILLE.
#
...
dire() { curl -fsS -m 20 -H "Title: q85 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
cp -f "$LOG" "$COURRIER_RES/q85_log.txt" 2>/dev/null || : > "$COURRIER_RES/q85_log.txt"
{
...
} > "$COURRIER_RES/q85_extrait.txt" 2>&1

if [ -z "$TROUVE" ]; then
    dire "ko" "pas de quete apres ${DUREE}s (vu=$VU) — extrait + log entier en pieces jointes"
    for f in "$COURRIER_RES/q85_extrait.txt:q85_extrait.txt" "$COURRIER_RES/q85_log.txt:q85_log.txt"; do
        curl -fsS -m 120 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q85 ${f##*:}" "$NT" >/dev/null 2>&1
        sleep 3
    done
    exit 1
fi
...
python3 "$GD/tools/scenarios/valider.py" "$TROUVE" > "$COURRIER_RES/verdict85.txt" 2>&1
VERDICT=$?
...
for f in "$TROUVE:q85_quete.json" "$COURRIER_RES/verdict85.txt:q85_verdict.txt" "$COURRIER_RES/q85_extrait.txt:q85_extrait.txt"; do
    curl -fsS -m 90 --retry 2 -T "${f%%:*}" -H "Filename: ${f##*:}" -H "Title: q85 ${f##*:}" "$NT" >/dev/null 2>&1
    sleep 2
done
echo "job-085 : ${DUREE}s, $DIAG"
