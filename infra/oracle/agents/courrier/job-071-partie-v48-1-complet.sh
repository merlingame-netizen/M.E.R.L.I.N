#!/usr/bin/env bash
# job-071 — LA PARTIE TEMOIN DE v48.1 COMPLET (sept moities), jugee sur les trois cibles dures.
#
...
dire() { curl -fsS -m 20 -H "Title: p71 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
...
# Le mode couvrant a-t-il vraiment tourne ? La sonde l'imprime ; on ne suppose pas.
if ! grep -aq "choix des cartes : COUVRANT" "$COURRIER_RES/partie.log" 2>/dev/null; then
    dire "ko" "mode couvrant JAMAIS engage : $(grep -a 'choix des cartes' "$COURRIER_RES/partie.log" 2>/dev/null | head -c 110)"
    exit 1
fi
if [ ! -s "$B/journal.json" ]; then
    tail -40 "$COURRIER_RES/partie.log" > "$COURRIER_RES/pourquoi71.txt"
...
python3 "$AGENTS/courrier/verdict_partie.py" "$B/journal.json" > "$COURRIER_RES/verdict71.txt" 2>&1
dire "verdict" "captures=$npng sha=$SHA $(head -c 800 "$COURRIER_RES/verdict71.txt")"
curl -fsS -m 60 --retry 2 -T "$COURRIER_RES/journal.json" \
    -H "Filename: p71_journal.json" -H "Title: p71 journal" "$NT" >/dev/null 2>&1
echo "job-071 : partie jouee (sha=$SHA), $npng captures + journal dans COURRIER_RES."
