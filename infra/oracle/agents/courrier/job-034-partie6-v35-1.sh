#!/usr/bin/env bash
# Partie témoin v35.1 : cible >= 1 scène lookahead SERVIE (enfin), le reste maintenu.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p34 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; }

deadline=$(( $(date +%s) + 1500 ))
force_fait=0
while ! grep -q 'plein_regime": true,' "$GD/scripts/llm/merlin_prompt_builder.gd" 2>/dev/null; do
    if [ "$(date +%s)" -ge "$deadline" ]; then
        dire "ko" "v35.1 jamais deployee ($(git -C "$GD" rev-parse --short HEAD 2>/dev/null))"
        exit 1
    fi
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "validation v35.1" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
split -b 250 -d -a 3 "$B/journal.json" /tmp/j34.
total=$(ls /tmp/j34.* | wc -l | tr -d ' ')
i=0
for p in $(ls /tmp/j34.* | sort); do
    i=$((i+1))
    curl -fsS -m 30 --retry 2 -H "Title: journal34 part $i/$total" \
