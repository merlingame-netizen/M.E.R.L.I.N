#!/usr/bin/env bash
# Re-partie témoin v35.3 : sélection VALIDÉE (ok:true + sentiers non vides, une relance
# sinon), puis 6 beats — cible : la première lookahead SERVIE, acquis maintenus.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p40 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; }
sel_valide() {
...
force_fait=0
while ! grep -q "sentiers.is_empty()" "$GD/tools/probe_partie_journal.gd" 2>/dev/null; do
    if [ "$(date +%s)" -ge "$deadline" ]; then
        dire "ko" "v35.3 jamais deployee"
        exit 1
    fi
...
while :; do
    ...
    dire "sel-ko" "essai $essais invalide : $(head -c 300 "$B/selection.json" 2>/dev/null | tr '\n' ' ')"
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "re-temoin v35.3" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
split -b 250 -d -a 3 "$B/journal.json" /tmp/j40.
total=$(ls /tmp/j40.* | wc -l | tr -d ' ')
i=0
for p in $(ls /tmp/j40.* | sort); do
    i=$((i+1))
    curl -fsS -m 30 --retry 2 -H "Title: journal40 part $i/$total" \
