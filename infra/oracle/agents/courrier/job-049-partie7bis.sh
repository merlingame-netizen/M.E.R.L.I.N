#!/usr/bin/env bash
# Partie témoin v35.4 — 3e tentative (p48 tuée par la boucle sans grâce, corrigée) :
# verdict + autopsie à chaud du log natif (scènes lookahead : lancées ? finies ? jetées ?).
set -u
...
dire() { curl -fsS -m 20 -H "Title: p49 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t49.
    local total i p
    total=$(ls /tmp/t49.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t49.* | sort); do
        i=$((i+1))
...
grep -q "GRÂCE DE DÉMARRAGE" "$AGENTS/a_partie_journal.sh" 2>/dev/null || { dire "ko" "runner sans grace — outillage pas a jour"; exit 1; }
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "temoin v35.4 ter : grace au depart, autopsie a chaud" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
tranches "course49" "$D"
tranches "journal49" "$B/journal.json"
echo "p49 : verdict + course + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
