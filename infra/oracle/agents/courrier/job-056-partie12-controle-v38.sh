#!/usr/bin/env bash
# Partie de CONTRÔLE v38 — après retrait du chaînage lookahead.
# Cibles : SECOURS=0, issue_s_moy <= 40 s, duree_moy <= 50 s, prov sans banc.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p56 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t56.
    local total i p
    total=$(ls /tmp/t56.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t56.* | sort); do
        i=$((i+1))
...
while ! grep -q "v38" "$GD/scripts/game/merlin_game.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v38 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "controle v38 : sans lookahead" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
tranches "journal56" "$B/journal.json"
echo "p56 : verdict + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
