#!/usr/bin/env bash
# Banc BitNet, prise 4 (la dernière) : g++ installé en user-mode par le pattern
# gamestack (dnf download --resolve + rpm2cpio). Échec => piste BitNet close, dit.
set -u
NT="https://ntfy.sh/merlin-courrier-vX9k2Qf7Lw3s"
W="$HOME/.cache/bitnet-banc"
SR="$W/toolchain"
dire() { curl -fsS -m 20 -H "Title: bit37 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; }

export PATH="$HOME/.local/cmake/bin:$HOME/.local/bin:$PATH"
if [ ! -x "$SR/usr/bin/g++" ]; then
    mkdir -p "$SR/rpms"
    cd "$SR/rpms"
    dire "etape" "$(date -u +%H:%M:%SZ) dnf download gcc-c++"
    dnf download --resolve --downloaddir=. gcc-c++ >> "$COURRIER_RES/build.log" 2>&1 \
        || { dire "ko" "dnf download KO : $(tail -c 300 "$COURRIER_RES/build.log" | tr '\n' ' ')"; exit 1; }
    cd "$SR"
    for r in rpms/*.rpm; do
        rpm2cpio "$r" | cpio -idm >> "$COURRIER_RES/build.log" 2>&1
    done
fi
[ -x "$SR/usr/bin/g++" ] || { dire "ko" "g++ absent apres extraction — piste BitNet close"; exit 1; }
dire "etape" "$(date -u +%H:%M:%SZ) g++ $($SR/usr/bin/g++ --version | head -1)"

cd "$W/BitNet" || { dire "ko" "clone absent"; exit 1; }
rm -rf build
export CXX="$SR/usr/bin/g++"
export CC="/usr/bin/cc"
export CPLUS_INCLUDE_PATH="$SR/usr/include/c++/11:$SR/usr/include/c++/11/aarch64-redhat-linux:${CPLUS_INCLUDE_PATH:-}"
export LIBRARY_PATH="$SR/usr/lib/gcc/aarch64-redhat-linux/11:${LIBRARY_PATH:-}"
export LD_LIBRARY_PATH="$SR/usr/lib64:${LD_LIBRARY_PATH:-}"
{ cmake -B build -DBITNET_ARM_TL1=ON >> "$COURRIER_RES/build.log" 2>&1 \
    ...
    || { dire "ko" "build KO malgre g++ : $(tail -c 400 "$COURRIER_RES/build.log" | tr '\n' ' ') — piste BitNet close"; exit 1; }
[ -x build/bin/llama-cli ] || { dire "ko" "llama-cli absent — piste close"; exit 1; }
...
bench() {
    ...
    for p in $(ls /tmp/bn.* | sort); do
        i=$((i+1))
        curl -fsS -m 30 -H "Title: bit37-$nom part $i/$total" --data-binary @"$p" "$NT" >/dev/null 2>&1
        sleep 2
    done
    rm -f /tmp/bn.*
}
