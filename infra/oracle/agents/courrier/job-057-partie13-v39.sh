#!/usr/bin/env bash
# Partie de contrôle v39 — l'arc s'efface devant les issues.
# Cibles : beat1 <= 50 s, duree_moy <= 40 s, issue_s_moy <= 30 s, SECOURS=0, arc:6.
set -u
...
dire() { curl -fsS -m 20 -H "Title: p57 $1" --data-binary "$2" "$NT" >/dev/null 2>&1; sleep 3; }
tranches() {
    split -b 250 -d -a 3 "$2" /tmp/t57.
    local total i p
    total=$(ls /tmp/t57.* | wc -l | tr -d ' ')
    i=0
    for p in $(ls /tmp/t57.* | sort); do
        i=$((i+1))
...
while ! grep -q "v39" "$GD/scripts/llm/merlin_scenario.gd" 2>/dev/null; do
    [ "$(date +%s)" -ge "$deadline" ] && { dire "ko" "v39 jamais deployee"; exit 1; }
    sleep 30
done
...
MERLIN_BEATS=6 env -u RES bash "$AGENTS/a_partie_journal.sh" partie 0 \
    "controle v39 : l'arc cede aux issues" > "$COURRIER_RES/partie.log" 2>&1
RCP=$?
...
fin = d.get("fin") or {}
print("beats=%d SECOURS=%d prov=%s beat1=%.0fs duree_moy=%.0fs issue_tok_moy=%.0f issue_s_moy=%.0fs fin=%s corr=%s titre=%s" % (
...
tranches "journal57" "$B/journal.json"
echo "p57 : verdict + journal envoyes via $NT"
[ "$RCP" -eq 0 ]
