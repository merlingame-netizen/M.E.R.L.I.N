#!/usr/bin/env bash
# LA PARTIE TÉMOIN — joue une vraie partie sur la VM, avec le moteur du jeu, et en tient le journal.
#
# HÉBERGEMENT SEUL (2026-09-08) : ce n'est plus un agent planifié ; c'est un outil que le Courrier
# appelle à la demande (job-104 : la chronique imagée d'une partie entièrement générée). Deux
# phases, comme avant : `selection` écrit les trois sentiers du modèle, `partie N` en joue un.
#   bash partie-temoin.sh selection
#   bash partie-temoin.sh partie 0 "motif du choix"
# Variables : MERLIN_BIOME, MERLIN_BEATS (vide = longueur libre), MERLIN_BOT_COUVRANT=1,
# MERLIN_SHOTS_MAX (clichés, défaut 12). Résultats dans ~/.cache/merlin-partie/.
...
trap 'printf stopped > "$HOME/.cache/merlin-game/desired"; bash "$HERE/game-stack.sh" stop >/dev/null 2>&1; exit 143' TERM INT
...
etape 1 4 "le jeu s'arrête, s'il tournait"
